package burujiyaseer.example.tmdbapp.ui
...
class MovieViewHolder(val itemBinding: MovieItemBinding) :
    ...
    fun bindMovie(movie: Movie, context: Context) {
        ...
        Glide.with(itemBinding.moviePoster.context).load(imageBase + movie.poster)
            .into(itemBinding.moviePoster)
        if (movie.ratings?.toFloat()!! > 7.0) {
            itemBinding.movieRatings.background = ResourcesCompat.getDrawable(
                context.resources, R.drawable.background_ratings, null
            )
        } else if (movie.ratings.toFloat() > 5.0) {
            itemBinding.movieRatings.background = ResourcesCompat.getDrawable(
                context.resources, R.drawable.background_average_ratings, null
            )
        } else {
            itemBinding.movieRatings.background = ResourcesCompat.getDrawable(
                context.resources, R.drawable.background_bad_ratings, null
            )
        }
    }
}
